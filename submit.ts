...
async function submit() {
  ...
  log(`Submitting: Year: ${mostRecent.problem.year}, Day: ${mostRecent.problem.day}, Part: ${part}, Answer: ${answer}`);
  ...
  if (result.status === 200) {
    ...
    if (responseText.includes("That's not the right answer")) {
      ...
      log(`Cooldown in progress (${waitTime} remaining). Re-submitting now will auto wait for cooldown to expire.`);
